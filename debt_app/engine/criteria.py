...
def _sanitize_dmp_checklist(checklist: Optional[dict]) -> Optional[dict]:
    ...
    reads it (assess_case here, build_dmp_checklist in views/criteria/assess.py). Returns a
...
def _compute_majority_analysis(case: dict, positions: list, council_positions: list = None, estimated_dividend_pence: int = 0) -> dict:  # EXCEL_CRITERIA_REFERENCE.md — Council Majority / DO_NOT_VOTE denominator rule
    ...
    # Debt held by creditors who abstain. DO_NOT_VOTE means "submits proof of
    # debt but does not vote" (EXCEL_CRITERIA_REFERENCE.md — GENERAL CREDITOR
    # status table), so these balances are excluded from BOTH the numerator
    # (voting_debt, below) and the 75% base.
    #
    # Derived by SUBTRACTING from `total_debt` rather than re-summing
    # `creditors`: `total_debt` is unsecured-only (see _parse_case) while
    # `creditors` also carries secured debt, so summing the list directly would
    # pull a mortgage into the threshold base. The secured guard here keeps the
    # subtrahend on the same unsecured footing as the total it comes off.
    abstaining_debt = sum(
        (c["crm_balance"] for c in creditors
         if _is_do_not_vote(c) and not c.get("is_secured")),
        Decimal("0"),
    )
    ...
    # The 75% majority is measured against the creditors who actually VOTE, not
    # against every pound of debt in the case — an abstention is not a NO.
    # EXCEL_CRITERIA_REFERENCE.md's Council Majority rule fails a case when
    # councils "are likely to vote NO and represent >25% of debt by value";
    # a council that submits a proof of debt and then does not vote is not that.
    # Holding abstainers in the base made the threshold unreachable by
    # construction on any case where non-voting creditors held more than a
    # quarter of the book, producing a false MAJORITY-IMPOSSIBLE hard block.
    #
    # An empty voting pool (every creditor abstains) yields threshold 0, so
    # `achievable` stays True and no block is raised — the same
    # "cannot compute a majority, so do not manufacture one" stance as the
    # zero-total guard above.
    voting_pool = max(Decimal("0"), full_total - abstaining_debt)
    threshold = (voting_pool * Decimal("0.75")).quantize(Decimal("0.01"), rounding=ROUND_HALF_UP)
    ...
    return {
        ...
        # The base the 75% threshold was taken from: total unsecured debt less
        # the balances of creditors who abstain. Surfaced so a caseworker can
        # see WHY the threshold is lower than 75% of the headline debt figure.
...
def assess_case(case_json: dict, detected_representatives: Optional[set] = None) -> dict:
        ...
        if majority_analysis.get("indeterminate"):
            flags.append(RuleResult(
                ...
                message=(
                    ...
                    f"£{majority_analysis['threshold']:,.2f} of the "
                    f"£{majority_analysis['voting_pool']:,.2f} of debt that will "
                    f"actually be voted must vote in favour. So far, only "
                    f"£{majority_analysis['voting_debt']:,.2f} of "
            ...
            # REFERRAL, not a hard block. An unreachable 75% is a fact about the
            # creditor book, not a defect in the case, and the engine's view of
            # who votes is an ESTIMATE built from stored creditor positions —
            # a creditor can vote against type, a rep body can be re-canvassed,
            # and debts get corrected. Auto-refusing removed the caseworker's
            # ability to say otherwise, so this is surfaced for review and the
            # decision to abandon or proceed stays with a human.
            #
            # The maths is untouched: this branch is still only reached when the
            # threshold cannot be met even counting EVERY undecided creditor as
            # a yes (see `indeterminate` above, which takes the other branch).
            flags.append(RuleResult(
                ...
                message=(
                    ...
                    f"£{majority_analysis['threshold']:,.2f} of the "
                    f"£{majority_analysis['voting_pool']:,.2f} of debt that will "
                    f"actually be voted must vote in favour. "
                    f"Even if every undecided creditor voted yes, only "
                    f"£{majority_analysis['voting_debt_optimistic']:,.2f} of support "
                    "could be reached, which is not enough. On these figures the case "
                    "cannot proceed as an IVA unless more creditor support is found — "
                    "a caseworker must review it before it goes any further."
                    + (
            ...
            # `overall` was computed from hard_blocks/flags BEFORE this rule ran,
            # so a case whose only finding is this one would otherwise report
            # "pass" while carrying a flag. Never downgrades an existing
            # "blocked" — only lifts a clean pass to flagged.
            if overall == "pass":
                overall = "flagged"
    ...
    #   - Aryza's native CaseData (integrations/aryza.py CaseData.dependants /
    ...
    from debt_app.engine.sfs import derive_household_key, get_guideline_rate, apply_guideline_constraint
